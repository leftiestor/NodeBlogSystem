...
function init(){
	let author = $("#categories").attr("data-postauthor");
	//获取关于信息 -博龄
    $.ajax({
        url: "/"+author+"/userInfo/statistic/registerInfo",
        type: "get",
        data: {},
        success: (result) => {
			let timeStr = "";
	...
	//获取关注和粉丝数量
	$.ajax({
        url: "/"+author+"/userInfo/statistic/caredInfo",
        type: "get",
        data: {},
        success: (result) => {
			if(result.code == "0"){
    ...
    //获取侧边栏信息
    $.ajax({
        url: "/getAsideCategory/"+author,
        type: "get",
        data: {},
        success: (result) => {
            let str = "";
            result.forEach((item, index) => {
                str += `<li><a href="/posts/category/${item._id.toString()}">${item.name}</a></li>`;
            });
            $("#categories").html(str);
        },
        error: (err) => {
